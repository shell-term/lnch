...
- **タスクごとのログ** — stdout/stderr をリングバッファに保持し、色分け表示。vim ライクな `/` 検索でインクリメンタルハイライト
- **グレースフルシャットダウン** — SIGTERM → タイムアウト後に SIGKILL。プロセスグループにより孤児プロセスを防止
- **クロスプラットフォーム** — macOS / Linux（WSL含む）/ Windows 対応
- **アップデート通知** — 起動時に新しいリリースを自動チェック。`u` キーで TUI から直接アップデート実行
...
| バージョン | 内容 | ステータス |
|-----------|------|-----------|
| **v0.1** | MVP — YAML設定、TUI、プロセス管理、`depends_on` | ✅ 完了 |
| **v0.2** | ログ検索 + 設定リロード | 🚧 開発中 |
| **v0.3** | `lnch init` 対話型プロンプト | 計画中 |
| **v0.4** | ログファイル出力 | 計画中 |
| **v0.5** | 自動再起動ポリシー | 計画中 |
